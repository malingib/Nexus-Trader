...
import { Account, EquityPoint } from '../types';
...
import { USER_PROFILE } from '../services/mockData';
...
const Dashboard: React.FC<DashboardProps> = ({ accounts, equityData }) => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 glass-card rounded-3xl p-8 relative overflow-hidden group border border-white/5">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 relative z-10">
            ...
            <div className="mt-4 md:mt-0 px-4 py-2 rounded-xl bg-gray-900/80 border border-white/10 text-gray-300 font-mono text-sm flex items-center gap-3 shadow-inner">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 relative z-10">
            <div className="bg-gray-900/40 rounded-2xl p-5 border border-white/5 hover:border-white/10 transition-colors">
              ...
              <span className="text-2xl font-bold text-white tracking-tight">${(totalBalance).toLocaleString()}</span>
            </div>
            <div className="bg-gray-900/40 rounded-2xl p-5 border border-white/5 hover:border-white/10 transition-colors">
              <div className="flex items-center gap-2 mb-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard 
            title="Total Profit" 
            value={`$${(totalDailyPL > 0 ? totalDailyPL : 0).toLocaleString()}`} 
            change="On Target" 
            isPositive={true} 
        ...
        <StatCard 
            title="Floating Loss" 
            value={`$${Math.abs(Math.min(0, totalFloating)).toLocaleString()}`} 
          ...
          <div className="h-[320px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={equityData}>
                ...
                <YAxis stroke="#6b7280" fontSize={11} axisLine={false} tickLine={false} domain={['auto', 'auto']} dx={-10} tickFormatter={(val) => `$${val/1000}k`} />
                <Tooltip 
                ...
                <tbody className="text-sm">
                    {[
                        ...
                        <tr key={i} className="group hover:bg-white/5 transition-colors border-b border-white/5 last:border-0">
                            ...
                            <td className={`py-4 font-bold font-mono ${row.pl.startsWith('+') ? 'text-emerald-400' : 'text-rose-400'}`}>{row.pl}</td>
...
const LimitCard = ({ label, current, max, color, icon, isDays }: any) => {
    const colorMap: any = {
        ...
        <div className="glass-card p-5 rounded-2xl border border-white/5 flex flex-col justify-between h-[100px] relative overflow-hidden group hover:border-white/10 transition-colors">
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-end mb-2">
                    <span className="text-xl font-bold text-white">{isDays ? `${current} Days` : `$${current.toLocaleString()}`}</span>
                    <span className="text-[10px] text-gray-500 font-mono">Target: {isDays ? max : `$${max.toLocaleString()}`}</span>
                </div>
